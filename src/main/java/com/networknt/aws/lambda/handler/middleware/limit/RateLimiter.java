...
import com.networknt.limit.LimitKey;
import com.networknt.limit.LimitQuota;
...
public class RateLimiter {
    ...
    public RateLimitResponse handleRequest(final LightLambdaExchange exchange, LimitKey limitKey) {
        ...
        synchronized(this) {
            for (LimitQuota limitQuota: rateLimit) {
                ...
                if (timeMap.isEmpty()) {
                    ...
                    Long countInOverallTime = removeOldEntriesForUser(currentTimeWindow, timeMap, limitQuota.getUnit());
                    if (countInOverallTime < limitQuota.getValue()) {
    ...
    private String getRateLimitReset(Long currentTimeWindow, Map<Long, AtomicLong> timeMap,  LimitQuota limitQuota) {
        String res = null;
        if (TimeUnit.SECONDS.equals(limitQuota.getUnit())){
            res = "1s";
        } else {
            Optional<Long> firstKey = timeMap.keySet().stream().findFirst();
            long reset = getWindow(limitQuota.getUnit()) + firstKey.get() - currentTimeWindow;
            res = reset + "s";
        }
        return res;
    }
    ...
    private Map<String, String> buildHeaders(Long countInOverallTime, LimitQuota limitQuota, String reset) {
        ...
        headers.put(Constants.RATELIMIT_LIMIT, limitQuota.getValue() + "/" + limitQuota.getUnit());
        headers.put(Constants.RATELIMIT_REMAINING, String.valueOf(limitQuota.getValue() - countInOverallTime));
        if (reset!=null) {
